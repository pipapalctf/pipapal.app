...
import { 
  ...
  insertEcoTipSchema,
  insertMaterialListingSchema,
  insertMaterialBidSchema,
  CollectionStatus,
  MaterialStatus,
  Collection,
  UserRole
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Material Marketplace - Material Listings
  
  // Get all available material listings (for recyclers)
  app.get("/api/materials/available", 
    requirePermission(Permissions.VIEW_MARKETPLACE),
    async (req, res) => {
      if (!req.user) return res.sendStatus(401);
      
      // Only recyclers can view available materials
      if (req.user.role !== UserRole.RECYCLER) {
        return res.status(403).json({
          error: 'Access denied',
          message: 'Only recyclers can view available materials'
        });
      }
      
      const materials = await storage.getAvailableMaterialListings();
      res.json(materials);
    }
  );
  
  // Get collector's own material listings
  app.get("/api/materials/my-listings", 
    requirePermission(Permissions.LIST_MATERIALS),
    async (req, res) => {
      if (!req.user) return res.sendStatus(401);
      
      // Only collectors can have their own listings
      if (req.user.role !== UserRole.COLLECTOR) {
        return res.status(403).json({
          error: 'Access denied',
          message: 'Only collectors can have material listings'
        });
      }
      
      const materials = await storage.getMaterialListingsByCollector(req.user.id);
      res.json(materials);
    }
  );
  
  // Get a specific material listing
  app.get("/api/materials/:id", 
    ...
    async (req, res) => {
      ...
      const material = await storage.getMaterialListing(id);
      if (!material) return res.status(404).send("Material listing not found");
      
      // For collectors, only allow viewing their own listings
      if (req.user.role === UserRole.COLLECTOR && material.collectorId !== req.user.id) {
        return res.status(403).json({
          error: 'Access denied',
          message: 'You do not have permission to view this material listing'
        });
      }
  ...
  // Create a new material listing (collectors only)
  app.post("/api/materials", 
    requirePermission(Permissions.LIST_MATERIALS),
    async (req, res) => {
      if (!req.user) return res.sendStatus(401);
      
      // Only collectors can create material listings
      if (req.user.role !== UserRole.COLLECTOR) {
        return res.status(403).json({
          error: 'Access denied',
          message: 'Only collectors can list materials'
        });
      }
      
      try {
        const materialData = insertMaterialListingSchema.parse({
          ...req.body,
          collectorId: req.user.id,
          status: MaterialStatus.AVAILABLE
        });
        
        const material = await storage.createMaterialListing(materialData);
        res.status(201).json(material);
      } catch (error) {
        console.error("Material listing creation error:", error);
        if (error instanceof z.ZodError) {
          return res.status(400).json({ errors: error.format() });
        }
        res.status(500).send("Failed to create material listing");
      }
    }
  );
  ...
  app.patch("/api/materials/:id", 
    ...
    async (req, res) => {
      ...
      if (material.collectorId !== req.user.id) {
        return res.status(403).json({
          error: 'Access denied',
          message: 'You do not have permission to update this material listing'
        });
      }
      
      try {
        const updates = { ...req.body };
        const updatedMaterial = await storage.updateMaterialListing(id, updates);
        res.json(updatedMaterial);
      } catch (error) {
        console.error("Material listing update error:", error);
        res.status(500).send("Failed to update material listing");
      }
    }
  );
  
  // Material Marketplace - Bids
  
  // Get bids for a material listing
  app.get("/api/materials/:id/bids", 
    requirePermission(Permissions.VIEW_MARKETPLACE),
    async (req, res) => {
      if (!req.user) return res.sendStatus(401);
      ...
      // For collectors, only allow viewing bids on their own listings
      if (req.user.role === UserRole.COLLECTOR && material.collectorId !== req.user.id) {
        return res.status(403).json({
          error: 'Access denied',
          message: 'You do not have permission to view bids for this material'
        });
      }
      
      const bids = await storage.getMaterialBidsByMaterial(id);
      res.json(bids);
    }
  );
  
  // Get recycler's own bids
  app.get("/api/materials/bids/my-bids", 
    requirePermission(Permissions.VIEW_MARKETPLACE),
    async (req, res) => {
      if (!req.user) return res.sendStatus(401);
      
      // Only recyclers can place bids
      if (req.user.role !== UserRole.RECYCLER) {
        return res.status(403).json({
          error: 'Access denied',
          message: 'Only recyclers can view their own bids'
        });
      }
      
      const bids = await storage.getMaterialBidsByRecycler(req.user.id);
      res.json(bids);
    }
  );
  
  // Create a new bid (recyclers only)
  app.post("/api/materials/:id/bids", 
    requirePermission(Permissions.VIEW_MARKETPLACE),
    async (req, res) => {
      if (!req.user) return res.sendStatus(401);
      ...
      if (isNaN(materialId)) return res.status(400).send("Invalid material ID format");
      
      const material = await storage.getMaterialListing(materialId);
      if (!material) return res.status(404).send("Material listing not found");
      ...
      if (material.status !== MaterialStatus.AVAILABLE) {
        return res.status(400).json({
          ...
          message: 'This material is no longer available for bidding'
        });
      }
      
      try {
        const bidData = insertMaterialBidSchema.parse({
          ...req.body,
          materialId,
        ...
        const bid = await storage.createMaterialBid(bidData);
        
        // Notify collector about new bid
        const collectorClients = clients.get(material.collectorId) || [];
        if (collectorClients.length > 0) {
          const notification = {
            type: 'new_bid',
            materialId: material.id,
            bidId: bid.id,
            message: `You received a new bid of KES ${bid.amount} for your ${material.materialType} listing`
          };
  ...
  // Update a bid (accept/reject)
  app.patch("/api/materials/bids/:id", 
    requirePermission(Permissions.LIST_MATERIALS),
    async (req, res) => {
      if (!req.user) return res.sendStatus(401);
      
      const id = parseInt(req.params.id);
      if (isNaN(id)) return res.status(400).send("Invalid bid ID format");
      
      const bid = await storage.getMaterialBid(id);
      if (!bid) return res.status(404).send("Bid not found");
      
      const material = await storage.getMaterialListing(bid.materialId);
      if (!material) return res.status(404).send("Material listing not found");
      
      // Only the material owner can update bids
      if (material.collectorId !== req.user.id) {
        return res.status(403).json({
          error: 'Access denied',
      ...
      try {
        const updates = { ...req.body };
        const updatedBid = await storage.updateMaterialBid(id, updates);
        
        // Notify recycler about bid status update
        const recyclerClients = clients.get(bid.recyclerId) || [];
        if (recyclerClients.length > 0 && updates.status) {
          const notification = {
            type: 'bid_update',
            materialId: material.id,
            bidId: bid.id,
            status: updates.status,
            message: `Your bid for ${material.materialType} has been ${updates.status}`
          };
